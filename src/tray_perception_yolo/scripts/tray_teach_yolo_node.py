...
WINDOW_NAME = "tray_teach_yolo_view"
BIN_CAMERA_COLOR_TOPIC = "/robot_camera/color/image_raw"
BIN_CAMERA_DEPTH_TOPIC = "/robot_camera/depth/image_raw"
BIN_CAMERA_INFO_TOPIC = "/robot_camera/color/camera_info"
BIN_CAMERA_CONTROL_SERVICE_ROOT = "/robot_camera"
...
MANUAL_TRAY_ROI_SOURCE = "manual_points"
GET_ANGLE_REASON_ITEM_SETUP = "tray setup"
...
class TrayTeachYoloNode(Node):
    def __init__(self) -> None:
        super().__init__("tray_teach_yolo")
        ...
        self.item_name = str(
            self.declare_parameter(
                "tray_name",
                self.declare_parameter("item_name", "").value,
            ).value
        ).strip()
        ...
        self.runtime_root = resolve_path(
            self.declare_parameter(
                "runtime_root",
                str(workspace_path("config", "tray_perception_yolo", "tray_teach_yolo_runtime")),
            ).value)
        self.saved_sessions_root = resolve_path(
            self.declare_parameter(
                "saved_sessions_root",
                str(workspace_path("config", "tray_perception_yolo", "tray_teach_yolo_saved_sessions")),
            ).value)
        self.runtime_settings_path = resolve_path(
            self.declare_parameter(
                "runtime_settings_path",
                str(workspace_path("config", "tray_perception_yolo", "tray_teach_yolo_runtime_settings.yaml")),
            ).value)
        ...
        self.profile_dir = resolve_path(
            self.declare_parameter(
                "profile_dir",
                str(workspace_path("teach", "tray_teach_yolo")),
            ).value)
        ...
        self.overlay_topic = self.declare_parameter("overlay_topic", "tray_overlay").value
        ...
        self.projection_camera_frame = str(
            self.declare_parameter("projection_camera_frame", "arm_calibrated_camera_link").value or ""
        ).strip().strip("/")
        self.calibration_parent_frame = "Link6"
        self.calibration_child_frame = "arm_calibrated_camera_link"
        ...
        self.yolo_preview_enabled = as_bool(self.declare_parameter("yolo_preview_enabled", True).value)
        self.yolo_preview_conf = float(self.declare_parameter("yolo_preview_conf", 0.35).value)
        self.yolo_preview_iou = float(self.declare_parameter("yolo_preview_iou", 0.45).value)
        self.yolo_preview_imgsz = int(self.declare_parameter("yolo_preview_imgsz", 640).value)
        self.yolo_preview_max_hz = float(self.declare_parameter("yolo_preview_max_hz", 3.0).value)
        ...
        self.tray_roi_selection_active = False
        self.tray_roi_points: List[Tuple[float, float]] = []
        ...
        self.frozen_depth_m: Optional[np.ndarray] = None
        self.frozen_camera_info: Optional[CameraInfo] = None
        ...
        self.status = "Enter tray name, capture tray samples, then train/import YOLO"
        ...
        self.yolo_preview_model = None
        self.yolo_preview_model_path = ""
        self.yolo_preview_detections: List[TeachYoloDetection] = []
        self.yolo_preview_status = "YOLO preview waiting for model"
        self.yolo_preview_last_inference_time = 0.0
        self.yolo_preview_paused_for_depth_plane = False
        ...
        self.tray_plane_selection_active = False
        self.tray_plane_auto_save_after_capture = False
        self.tray_plane_points: List[Tuple[float, float]] = []
        self.tray_plane_model: Optional[Tuple[float, float, float, float]] = None
        self.tray_width_mm = 0.0
        self.tray_height_mm = 0.0
        ...
        self.get_logger().info(f"tray_teach_yolo session: {self.session_dir}")
    ...
    def load_runtime_settings(self) -> None:
        ...
        try:
            ...
            params = root.get("tray_teach_yolo_runtime", {}).get("ros__parameters", {})
            if not params:
    ...
    def create_session_dir(self) -> Path:
        ...
        name = f"{safe_name(self.item_name, fallback='unnamed_tray')}_{timestamp_for_path()}"
    ...
    def runtime_root_is_safe_to_clear(self, path: Path) -> bool:
        ...
        try:
            ...
            pass
        return path.name in ("runtime", "item_teach_yolo_runtime", "tray_teach_yolo_runtime") and len(path.parts) >= 3
    ...
    def reset_runtime_for_item_name(self, new_name: str) -> None:
        ...
        self.tray_roi_selection_active = False
        self.tray_roi_points = []
        self.tray_plane_selection_active = False
        self.tray_plane_auto_save_after_capture = False
        self.tray_plane_points = []
        self.tray_plane_model = None
        self.tray_width_mm = 0.0
        self.tray_height_mm = 0.0
        self.frozen_depth_m = None
        ...
        self.refresh_video_recordings()
        self.write_dataset_yaml()
        self.status = f"Tray name changed to {self.item_name}; new session created"
        self.save_session()
        self.request_teach_joint_snapshot_for_item_setup()

    def write_dataset_yaml(self) -> None:
        class_name = self.item_name.strip() if self.has_item_name() else "tray"
        data = {
    ...
    def save_session(self) -> None:
        ...
        data = {
            "tray_teach_yolo_session": {
                "tray_name": self.item_name,
                ...
                "tray_roi_points_source": active_bin.roi_points_source if active_bin else "",
                ...
                "tray_plane_enabled": self.has_tray_plane_reference(),
                "tray_plane_a": self.tray_plane_model[0] if self.tray_plane_model else 0.0,
                "tray_plane_b": self.tray_plane_model[1] if self.tray_plane_model else 0.0,
                "tray_plane_c": self.tray_plane_model[2] if self.tray_plane_model else 0.0,
                "tray_plane_reference_depth_m": self.tray_plane_model[3] if self.tray_plane_model else 0.0,
                "tray_plane_roi_points": [
                    int(round(value))
                    for point in self.tray_plane_points
                    for value in point
                ],
    ...
    def request_teach_joint_snapshot_for_item_setup(self) -> None:
        if not self.has_item_name() or not self.has_manual_tray_roi():
            return
    ...
    def import_review_media(self, media_path: Path) -> Optional[Path]:
        ...
        if not is_image and not is_video:
            ...
            return None
        self.capture_images_dir.mkdir(parents=True, exist_ok=True)
        item_token = safe_name(self.item_name, fallback="tray")
    ...
    def capture_roi_image(self) -> None:
        if self.active_yolo_model_path() is not None:
            self.capture_frozen_tray_plane_frame()
            return
        if self.review_mode:
        ...
        if not self.has_item_name():
            ...
            return
        if not self.has_manual_tray_roi():
            self.status = "Teach tray ROI before capturing"
            return
    ...
    def start_video_recording(self) -> None:
        ...
        if not self.has_item_name():
            ...
            return
        if not self.has_manual_tray_roi():
            self.status = "Teach tray ROI before recording"
            return
    ...
    def load_saved_review_mask(self, frame: Dict) -> str:
        ...
        return f" | saved samples {total_count} tray {item_count} BG {background_count}"
    ...
    def read_saved_session_entry(self, path: Path) -> Optional[SavedSessionEntry]:
        ...
        try:
            ...
            params = root.get("tray_teach_yolo_session", {})
            if not isinstance(params, dict) or not params:
                params = root.get("item_teach_yolo_session", {})
            if not isinstance(params, dict):
                return None
            item_name = str(params.get("tray_name", params.get("item_name", path.name))).strip() or path.name
            ...
            label = (
                f"{item_name} | Tray {sample_count} BG {background_count} | "
                f"{modified_text}"
            )
    ...
    def restore_active_bin_from_session(self, params: Dict) -> None:
        self.active_bin_index = -1
        self.active_bin = None
        manual_roi_points = parse_flat_points(params.get("tray_roi_points", []))
        manual_roi_source = str(params.get("tray_roi_points_source", "")).strip()
        if len(manual_roi_points) == 4 and manual_roi_source == MANUAL_TRAY_ROI_SOURCE:
            self.set_manual_tray_roi(manual_roi_points, reset_plane=False, save=False)
            return
    ...
    def load_saved_session(self, path: Path) -> None:
        ...
        try:
            ...
            self.configure_session_storage()
            self.item_name = str(params.get("tray_name", params.get("item_name", ""))).strip()
            self.item_name_edit_buffer = self.item_name
            self.item_name_edit_active = False
            ...
            if as_bool(params.get("tray_plane_enabled", False)):
                try:
                    self.tray_plane_model = (
                        float(params.get("tray_plane_a", 0.0)),
                        float(params.get("tray_plane_b", 0.0)),
                        float(params.get("tray_plane_c", 0.0)),
                        float(params.get("tray_plane_reference_depth_m", 0.0)),
                    )
                    plane_points = parse_flat_points(params.get("tray_plane_roi_points", []))
                    self.tray_plane_points = plane_points if len(plane_points) == 4 else []
                    self.tray_width_mm = float(params.get("tray_width_mm", 0.0))
                    self.tray_height_mm = float(params.get("tray_height_mm", 0.0))
                except (TypeError, ValueError):
                    self.tray_plane_model = None
                    self.tray_plane_points = []
                    self.tray_width_mm = 0.0
    ...
    def set_manual_tray_roi(
        self,
        points: List[Tuple[float, float]],
        reset_plane: bool = True,
        save: bool = True,
    ) -> None:
        if len(points) != 4:
            self.status = "Tray ROI needs 4 ordered points"
            return
        roi_points = [(float(x), float(y)) for x, y in points]
        self.active_bin = BinEntry(
            label="Tray manual ROI",
            path=self.profile_dir / "tray_manual_roi.yaml",
            bin_name="tray_manual_roi",
            teach_date="",
            roi_points=roi_points,
            roi_points_source=MANUAL_TRAY_ROI_SOURCE,
        )
        self.active_bin_index = 0
        self.tray_roi_points = roi_points
        self.tray_roi_selection_active = False
        self.bin_dropdown_open = False
    ...
    def has_manual_tray_roi(self) -> bool:
        return (
            self.active_bin is not None
            and self.active_bin.roi_points_source == MANUAL_TRAY_ROI_SOURCE
            and len(self.active_bin.roi_points) == 4
        )

    def start_tray_roi_selection(self) -> None:
        if self.training_thread is not None and self.training_thread.is_alive():
            self.status = "Cannot teach ROI while training"
            return
        if self.recording_active:
            self.status = "Stop ROI capture before teaching ROI"
            return
        self.tray_roi_selection_active = True
        self.tray_plane_selection_active = False
        self.tray_plane_auto_save_after_capture = False
        self.tray_roi_points = []
        self.frozen_depth_m = None
        self.frozen_camera_info = None
        self.yolo_preview_paused_for_depth_plane = False
        self.status = "Tray ROI: click 4 ordered tray ROI corners"

    def handle_tray_roi_click(self, event: int, x: int, y: int) -> bool:
        if not self.tray_roi_selection_active:
            return False
        if event == cv2.EVENT_RBUTTONDOWN:
            self.tray_roi_selection_active = False
            self.tray_roi_points = []
            self.status = "Tray ROI teach canceled"
            self.save_session()
            return True
        if event != cv2.EVENT_LBUTTONDOWN:
            return True
        point = self.preview_point_to_frame(x, y)
        if point is None:
            return True
        self.tray_roi_points.append((float(point[0]), float(point[1])))
        if len(self.tray_roi_points) >= 4:
            points = self.tray_roi_points[:4]
            self.set_manual_tray_roi(points, reset_plane=True, save=True)
            if self.active_yolo_model_path() is not None:
                self.status = "Tray ROI set; click Capture ROI to freeze depth frame"
            else:
                self.status = "Tray ROI set; capture samples, train/import YOLO, then Teach Plane"
        else:
            self.status = f"Tray ROI: click {len(self.tray_roi_points)}/4"
        return True

    def ensure_tray_full_frame_roi(self, frame: Optional[np.ndarray]) -> None:
        if self.active_bin is not None and self.active_bin.roi_points_source == MANUAL_TRAY_ROI_SOURCE:
            self.tray_roi_points = list(self.active_bin.roi_points)
    ...
    def parse_transform_values(transform: Dict) -> Tuple[Tuple[float, float, float], Tuple[float, float, float, float]]:
        translation_node = TrayTeachYoloNode.yaml_map(transform.get("translation", {}))
        rotation_node = TrayTeachYoloNode.yaml_map(transform.get("rotation", {}))
        translation = (
    ...
    def load_eye_to_hand_calibration(self, path: Path) -> bool:
        try:
            ...
            if normalize_calibration_type(params.get("calibration_type", "")) != "eye_in_hand":
                raise ValueError("expected parameters.calibration_type=eye_in_hand")
            ...
            parent = str(
                ...
                params.get("robot_tool_frame") or
            ...
            self.get_logger().warn(f"Failed to load eye-on-hand calibration {path}: {exc}")
            return False
    ...
    def load_platform_calibration(self, path: Path) -> bool:
        try:
            ...
            transform = self.yaml_map(root.get("transform", root.get("calibration_transform", {})))
            translation, rotation = self.parse_transform_values(transform)
    ...
    def load_and_publish_station_calibration_tfs(self) -> None:
        ...
        if eye_path is not None and self.load_eye_to_hand_calibration(eye_path):
            ...
            self.get_logger().info(
                f"YOLO tray teach loaded eye-on-hand TF {self.calibration_parent_frame}->{self.calibration_child_frame} "
                f"from {eye_path}")
    ...
    def update_active_bin_platform_projection(self) -> None:
        self.ensure_tray_full_frame_roi(self.latest_frame_copy())
    ...
    def color_callback(self, msg: Image) -> None:
        ...
        self.ensure_tray_full_frame_roi(bgr)
    ...
    def roi_views_from_frame(
        ...
        if len(self.active_bin.roi_points) < 3:
            self.status = "Waiting for tray camera frame"
            return None
        h, w = frame.shape[:2]
    ...
    def freeze_current_crop(self) -> bool:
        frame = self.latest_frame_copy()
        if frame is None:
            self.status = "Wait for color image"
            return False
        roi = self.roi_views_from_frame(frame)
        if roi is None:
            self.status = "Wait for color image"
        ...
        self.frozen_crop_bgr = crop
        self.frozen_crop_rgb = cv2.cvtColor(crop, cv2.COLOR_BGR2RGB)
        depth_m = self.latest_depth_for_frame_shape(frame.shape[:2])
        self.frozen_depth_m = None if depth_m is None else depth_m.copy()
        self.frozen_camera_info = copy.deepcopy(self.latest_camera_info) if self.latest_camera_info is not None else None
        self.roi_crop_rect = rect
    ...
    def latest_depth_for_frame_shape(self, shape: Tuple[int, int]) -> Optional[np.ndarray]:
    ...
    def capture_frozen_tray_plane_frame(self) -> bool:
        if self.review_mode:
            self.status = "Exit image review before capturing tray plane"
            return False
        if self.training_thread is not None and self.training_thread.is_alive():
            self.status = "Cannot capture tray plane while training"
            return False
        if self.recording_active:
            self.status = "Stop ROI recording before capturing tray plane"
            return False
        if not self.has_item_name():
            self.status = self.item_name_error()
            return False
        if not self.has_manual_tray_roi():
            self.status = "Teach tray ROI before capturing tray plane"
            return False
        if self.active_yolo_model_path() is None:
            self.status = "Train or open a YOLO .pt model before capturing tray plane"
            return False
        frame = self.latest_frame_copy()
        if frame is None:
            self.status = "Wait for color image before capturing tray plane"
            return False
        depth_m = self.latest_depth_for_frame_shape(frame.shape[:2])
        if depth_m is None:
            self.status = "Wait for depth image before capturing tray plane"
            return False
        if self.latest_camera_info is None or self.latest_camera_info.k[0] <= 1e-6 or self.latest_camera_info.k[4] <= 1e-6:
            self.status = "Wait for valid camera info before capturing tray plane"
            return False
        roi = self.roi_views_from_frame(frame)
        if roi is None:
            self.status = "Teach tray ROI before capturing tray plane"
        ...
        self.frozen_crop_bgr = crop
        self.frozen_crop_rgb = cv2.cvtColor(crop, cv2.COLOR_BGR2RGB)
        self.frozen_depth_m = depth_m.copy()
        self.frozen_camera_info = copy.deepcopy(self.latest_camera_info)
        self.roi_crop_rect = rect
        self.roi_crop_mask = roi_mask
        self.current_mask = None
        ...
        self.sam2_image_key = None
        self.prediction_dirty = False
        self.tray_plane_selection_active = True
        self.tray_plane_auto_save_after_capture = True
        self.tray_plane_points = []
        self.tray_plane_model = None
        self.tray_width_mm = 0.0
        self.tray_height_mm = 0.0
        self.yolo_preview_paused_for_depth_plane = True
        self.clear_yolo_preview_model("YOLO preview stopped for frozen depth plane teach")
        self.status = "Frozen ROI + depth; click 4 exposed tray plane points"
        self.save_session()
        return True

    def preview_point_to_frame(self, x: int, y: int) -> Optional[Tuple[int, int]]:
        ...
        if self.preview_source_size == (0, 0):
            return None
        frame_x = int(np.clip(frame_x, 0, max(0, self.preview_source_size[0] - 1)))
        frame_y = int(np.clip(frame_y, 0, max(0, self.preview_source_size[1] - 1)))
        return frame_x, frame_y

    def latest_depth_for_color_shape(self) -> Optional[np.ndarray]:
        if self.frozen_depth_m is not None:
            depth_m = self.frozen_depth_m
            frame = self.frozen_frame_bgr
        else:
            depth_m = self.latest_depth_m
            frame = self.latest_frame_copy()
        if depth_m is None or frame is None:
            return None
    ...
    @staticmethod
    def average_depth_at(depth_m: np.ndarray, point: Tuple[float, float], radius: int = 7) -> Optional[float]:
        x = int(round(point[0]))
        y = int(round(point[1]))
    ...
    def fit_tray_plane_from_points(self, points: List[Tuple[float, float]]) -> bool:
        depth_m = self.latest_depth_for_color_shape()
        if depth_m is None:
            self.status = "Depth frame unavailable for tray plane"
            return False
        if len(points) != 4:
            self.status = "Tray plane needs 4 ordered points"
            return False
        rows, cols = depth_m.shape[:2]
        if rows <= 1 or cols <= 1:
            self.status = "Invalid depth frame for tray plane"
            return False
        a_rows = []
        b_vals = []
        camera_points = []
        depth_sum = 0.0
        info = self.frozen_camera_info if self.frozen_camera_info is not None else self.latest_camera_info
        for point in points:
            depth = self.average_depth_at(depth_m, point, 7)
            if depth is None or not math.isfinite(depth) or depth <= 0.0:
                self.status = "Invalid depth at tray plane point"
                return False
            x = float(np.clip(point[0], 0.0, cols - 1))
            y = float(np.clip(point[1], 0.0, rows - 1))
            x_norm = x / float(cols - 1)
            y_norm = y / float(rows - 1)
            a_rows.append([x_norm, y_norm, 1.0])
            b_vals.append(depth)
            depth_sum += depth
            if info is not None and info.k[0] > 1e-6 and info.k[4] > 1e-6:
                camera_points.append(np.array([
                    (x - float(info.k[2])) * depth / float(info.k[0]),
                    (y - float(info.k[5])) * depth / float(info.k[4]),
                    depth,
                ], dtype=np.float64))
        try:
            ...
            self.status = f"Tray plane fit failed: {exc}"
            return False
        plane_a, plane_b, plane_c = [float(v) for v in solution]
        reference_depth_m = depth_sum / 4.0
        if not np.isfinite([plane_a, plane_b, plane_c, reference_depth_m]).all() or reference_depth_m <= 0.0:
            self.status = "Tray plane fit produced invalid coefficients"
            return False
        self.tray_plane_model = (plane_a, plane_b, plane_c, reference_depth_m)
        self.tray_plane_points = [(float(x), float(y)) for x, y in points]
        self.tray_width_mm = 0.0
        self.tray_height_mm = 0.0
        if len(camera_points) == 4:
            lengths = [
                float(np.linalg.norm(camera_points[(i + 1) % 4] - camera_points[i]) * 1000.0)
                for i in range(4)
            ]
            if all(math.isfinite(value) and value > 0.0 for value in lengths):
                side_a = 0.5 * (lengths[0] + lengths[2])
                side_b = 0.5 * (lengths[1] + lengths[3])
                self.tray_width_mm = max(side_a, side_b)
                self.tray_height_mm = min(side_a, side_b)
        if self.tray_width_mm <= 1.0 or self.tray_height_mm <= 1.0:
            self.status = "Tray plane set, but tray dimensions are unavailable"
            return False
        self.tray_plane_selection_active = False
        self.status = f"Tray plane set | size {self.tray_width_mm:.0f}x{self.tray_height_mm:.0f} mm"
        self.save_session()
        return True

    def has_tray_plane_reference(self) -> bool:
        return (
            self.tray_plane_model is not None
            and len(self.tray_plane_points) == 4
            and self.tray_width_mm > 1.0
            and self.tray_height_mm > 1.0
        )

    def start_tray_plane_selection(self) -> None:
        if self.training_thread is not None and self.training_thread.is_alive():
            self.status = "Cannot teach tray plane while training"
            return
        if not self.has_manual_tray_roi():
            self.status = "Teach tray ROI before teaching tray plane"
            return
        if self.active_yolo_model_path() is None:
            self.status = "Train or open a YOLO model before teaching tray plane"
            return
        self.tray_plane_selection_active = True
        self.tray_plane_auto_save_after_capture = False
        self.tray_plane_points = []
        ...
        self.tray_height_mm = 0.0
        self.yolo_preview_paused_for_depth_plane = True
        self.clear_yolo_preview_model("YOLO preview stopped for tray plane teach")
        self.status = "Tray plane: click 4 exposed ordered tray rectangle points"

    def handle_tray_plane_click(self, event: int, x: int, y: int) -> bool:
        if not self.tray_plane_selection_active:
            return False
        if event == cv2.EVENT_RBUTTONDOWN:
            self.tray_plane_selection_active = False
            self.tray_plane_auto_save_after_capture = False
            self.yolo_preview_paused_for_depth_plane = False
            self.status = "Tray plane teach canceled"
            self.save_session()
            return True
        if event != cv2.EVENT_LBUTTONDOWN:
            return True
        ...
        if point is None:
            return True
        self.tray_plane_points.append((float(point[0]), float(point[1])))
        if len(self.tray_plane_points) >= 4:
            points = self.tray_plane_points[:4]
            if self.fit_tray_plane_from_points(points) and self.tray_plane_auto_save_after_capture:
                self.tray_plane_auto_save_after_capture = False
                self.save_existing_model_teach()
        else:
            self.status = f"Tray plane: click {len(self.tray_plane_points)}/4"
        return True
    ...
    def save_sample(self) -> Optional[str]:
        if not self.has_item_name():
            ...
            return None
        if not self.has_manual_tray_roi():
            self.status = "Teach tray ROI before saving samples"
            return None
        if self.recording_active:
        ...
        self.status = f"Saved tray sample {self.sample_count} | {self.background_ratio_label()}"
        self.save_session()
        return stem
    ...
    def save_background_sample(self) -> None:
        if not self.has_item_name():
            ...
            return
        if not self.has_manual_tray_roi():
            self.status = "Teach tray ROI before saving background"
            return
        if self.recording_active:
        ...
        if crop_info is None:
            self.status = "Teach tray ROI and wait for color image"
            return
    ...
    def item_name_error(self) -> str:
        if not self.item_name.strip():
            return "Enter tray name first"
        return "Tray name needs letters or numbers"

    def sample_count_label(self) -> str:
        return (
            f"Tray {self.sample_count} | BG {self.background_sample_count} | "
    ...
    def active_yolo_model_path(self) -> Optional[Path]:
        if self.package_model_path is not None and self.package_model_path.exists():
            return self.package_model_path
        if self.trained_model_path:
            path = Path(self.trained_model_path)
            if path.exists():
                return path
        return None

    def clear_yolo_preview_model(self, status: str = "YOLO preview waiting for model") -> None:
        self.yolo_preview_model = None
        self.yolo_preview_model_path = ""
        self.yolo_preview_detections = []
        self.yolo_preview_status = status

    def ensure_yolo_preview_model(self) -> bool:
        model_path = self.active_yolo_model_path()
        if model_path is None:
            self.clear_yolo_preview_model()
            return False
        model_text = str(model_path)
        if self.yolo_preview_model is not None and self.yolo_preview_model_path == model_text:
            return True
        try:
            from ultralytics import YOLO
            self.yolo_preview_model = YOLO(model_text)
            self.yolo_preview_model_path = model_text
            self.yolo_preview_status = f"YOLO preview active: {model_path.name}"
            self.yolo_preview_detections = []
            return True
        except Exception as exc:
            self.clear_yolo_preview_model(f"YOLO preview unavailable: {exc}")
            self.get_logger().warn(self.yolo_preview_status)
            return False

    def run_yolo_preview(self, frame: np.ndarray) -> None:
        if not self.yolo_preview_enabled or frame.size == 0:
            self.yolo_preview_detections = []
            return
        if not self.ensure_yolo_preview_model() or self.yolo_preview_model is None:
            return
        now = time.monotonic()
        if self.yolo_preview_max_hz > 0.0 and now - self.yolo_preview_last_inference_time < 1.0 / self.yolo_preview_max_hz:
            return
        self.yolo_preview_last_inference_time = now
        try:
            results = self.yolo_preview_model.predict(
                source=frame,
                imgsz=self.yolo_preview_imgsz,
                conf=self.yolo_preview_conf,
                iou=self.yolo_preview_iou,
                classes=[0],
                device="cpu",
                verbose=False,
            )
            ...
            self.yolo_preview_detections = []
            self.yolo_preview_status = f"YOLO preview failed: {exc}"
            return
        self.yolo_preview_detections = self.parse_yolo_preview_result(
            results[0] if results else None,
            frame.shape[:2],
        )
        if self.yolo_preview_detections:
            best = self.yolo_preview_detections[0]
            self.yolo_preview_status = f"YOLO preview active | best {best.score:.2f}"
        else:
            self.yolo_preview_status = "YOLO preview active | no tray mask"

    def parse_yolo_preview_result(self, result, image_shape: Tuple[int, int]) -> List[TeachYoloDetection]:
        if result is None:
            return []
        ...
        for index, box_xyxy in enumerate(boxes_xyxy):
            ...
            if x2 <= x1 or y2 <= y1:
                continue
            mask = self.yolo_preview_mask(index, mask_polygons, mask_data, image_shape, (x1, y1, x2, y2))
            detections.append(TeachYoloDetection(
    ...
    def yolo_preview_mask(
        ...
        return mask if cv2.countNonZero(mask) >= 16 else None

    def draw_yolo_preview_overlay(self, frame: np.ndarray) -> None:
        if frame.size == 0 or not self.yolo_preview_detections:
            return
        overlay = frame.copy()
        for index, detection in enumerate(self.yolo_preview_detections[:3]):
            color = (56, 198, 232) if index == 0 else (92, 172, 210)
            if detection.mask is not None:
                overlay[detection.mask > 0] = color
            x, y, w, h = detection.box
            cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2, cv2.LINE_AA)
            label = f"YOLO tray {detection.score:.2f}"
    ...
    def can_save_existing_model_teach(self) -> bool:
        ...
        has_trained_model = bool(self.trained_model_path) and Path(self.trained_model_path).exists()
        return (
            not training_active and
            not self.recording_active and
            self.has_item_name() and
            self.has_manual_tray_roi() and
            self.has_tray_plane_reference() and
            (has_packaged_model or has_trained_model)
        )
    ...
    def unique_packaged_model_dir(self, item_token: str) -> Path:
        ...
        stem = f"tray_{item_token}_{compact_date_for_path()}"
    ...
    def tray_profile_params(
        ...
        train_epochs: int,
        train_device: str,
        train_device_used: str,
        packaged_from_model_path: str = "",
    ) -> Dict:
        if not self.has_item_name():
            raise RuntimeError("Enter tray name before saving YOLO teach.")
        if not self.has_manual_tray_roi():
            raise RuntimeError("Teach the tray ROI before saving YOLO teach.")
        if not self.has_tray_plane_reference() or self.tray_plane_model is None:
            raise RuntimeError("Teach the tray plane before saving YOLO teach.")
        plane_a, plane_b, plane_c, plane_ref = self.tray_plane_model
        date_stamp = compact_date_for_path()
        date_match = re.search(r"_(\d{8})(?:_\d+)?$", model_dir.name)
        if date_match:
            date_stamp = date_match.group(1)
        return {
            "detection_backend": f"{yolo_version}_seg_pt",
            "yolo_version": yolo_version,
            "model_family": yolo_version.upper(),
            "tray_name": self.item_name,
            "teach_date": date_stamp,
            "teach_date_compact": date_stamp,
            "created_at": _dt.datetime.now().isoformat(timespec="seconds"),
            "class_id": 0,
            ...
            "packaged_from_model_path": packaged_from_model_path,
            "sample_count": self.sample_count,
            "background_sample_count": self.background_sample_count,
            "total_training_image_count": self.total_training_image_count(),
            "background_ratio_percent": self.background_ratio_percent(),
            ...
            "train_device": self.train_device if train_epochs else "",
            "train_use_gpu_if_available": self.train_use_gpu_if_available,
            "train_device_used": train_device_used,
            "detection_mode": "yolo_tray_plane",
            "tray_dimension_source": "teach_plane_points",
            "tray_plane_enabled": True,
            "tray_plane_a": float(plane_a),
            "tray_plane_b": float(plane_b),
            "tray_plane_c": float(plane_c),
            "tray_plane_reference_depth_m": float(plane_ref),
            "tray_plane_roi": [
                int(round(min(point[0] for point in self.tray_plane_points))),
                int(round(min(point[1] for point in self.tray_plane_points))),
                int(round(max(point[0] for point in self.tray_plane_points))),
                int(round(max(point[1] for point in self.tray_plane_points))),
            ],
            "tray_plane_roi_points": [
                int(round(value))
                for point in self.tray_plane_points
            ...
            "tray_width_mm": float(self.tray_width_mm),
            "tray_height_mm": float(self.tray_height_mm),
            "motion_service_root": self.motion_service_root,
            "get_angle_service": self.get_angle_service_name,
            "teach_joints_deg": self.latest_joint_positions_deg if self.has_joint_positions else [],
            ...
            "roi_points": [
                int(round(value))
                for point in (self.active_bin.roi_points if self.active_bin else [])
                for value in point
            ],
            "roi_crop_rect": list(self.roi_crop_rect) if self.roi_crop_rect else [],
        }

    def write_tray_profile(
        self,
        model_dir: Path,
        model_path: Path,
        yolo_version: str,
        ...
        tray_token = safe_name(self.item_name)
        if not tray_token:
            raise RuntimeError("Enter tray name before saving YOLO teach.")
        params = self.tray_profile_params(
            model_dir,
            model_path,
            yolo_version,
            train_epochs,
        ...
        profile = {
            ...
            "tray_detect": {"ros__parameters": copy.deepcopy(params)},
            "tray_yolo": {"ros__parameters": copy.deepcopy(params)},
        }
    ...
    def write_existing_model_profile(self, model_dir: Path, model_path: Path) -> Path:
        return self.write_tray_profile(
            model_dir,
            model_path,
            self.package_yolo_version,
            0,
            "",
            "external_pt",
            str(self.package_model_path) if self.package_model_path else "",
        )
    ...
    def save_existing_model_teach(self) -> None:
        ...
        if not self.has_item_name():
            ...
            return
        if not self.has_manual_tray_roi():
            self.status = "Teach tray ROI before saving YOLO teach"
            return
        if not self.has_tray_plane_reference():
            self.status = "Teach tray plane before saving YOLO teach"
            return
        if self.package_model_path is None and not self.trained_model_path:
            self.status = "Open or train a YOLO .pt model before saving teach"
            return

        item_token = safe_name(self.item_name)
        model_dir: Optional[Path] = None
        try:
            if self.package_model_path is not None:
                ...
                final_pt = Path(self.trained_model_path)
                profile_path = self.write_tray_profile(
                    model_dir,
                    final_pt,
                    "yolo11",
                    self.train_epochs,
                    self.train_device,
                    self.train_device_used,
                )
        except Exception as exc:
            if self.package_model_path is not None and model_dir is not None:
        ...
        self.status = f"Saved YOLO tray teach: {model_dir.name}"
        self.save_session()
    ...
    def start_training(self) -> None:
        ...
        if not self.has_item_name():
            ...
            return
        if not self.has_manual_tray_roi():
            self.status = "Teach tray ROI before training"
            return
        if self.sample_count <= 0:
        ...
        self.training_progress = 0.0
        self.status = (
            f"Training YOLO11-seg on {self.sample_count} tray + "
    ...
    def train_worker(self) -> None:
        try:
            ...
            def on_train_start(trainer) -> None:
                ...
                self.status = (
                    f"Training tray YOLO11-seg on {self.training_device_label(train_device)}: "
            ...
            def on_fit_epoch_end(trainer) -> None:
                ...
                self.update_training_progress(completed, total_epochs, "training")
                self.status = (
                    f"Training tray YOLO11-seg on {self.training_device_label(train_device)}: epoch "
            ...
            self.status = "Training done: click Capture ROI to freeze depth plane"
        except Exception as exc:
    ...
    def teach_bundle_stem(self) -> str:
        item_token = safe_name(self.item_name)
        if not item_token:
            raise RuntimeError("Enter tray name before finalizing YOLO teach.")
        return f"tray_{item_token}_{compact_date_for_path()}"
    ...
    def promote_trained_model(self, best_path: Path) -> None:
        ...
        tray_token = safe_name(self.item_name)
        final_pt = model_dir / f"{tray_token}_yolo11.pt"
    ...
    def write_profile(self) -> None:
        if not self.final_model_dir or not self.trained_model_path or not self.has_tray_plane_reference():
            return
        model_dir = Path(self.final_model_dir)
        model_dir.mkdir(parents=True, exist_ok=True)
        self.write_tray_profile(
            model_dir,
    ...
    def draw_point_polygon_overlay(
        self,
        image: np.ndarray,
        points: List[Tuple[float, float]],
        ...
        if image.size == 0 or not points:
            return
        frame_points = [
            (
                int(np.clip(round(point[0]), 0, max(0, image.shape[1] - 1))),
                int(np.clip(round(point[1]), 0, max(0, image.shape[0] - 1))),
            )
            for point in points
        ]
        if len(frame_points) >= 2:
            ...
            if len(frame_points) == 4:
                cv2.line(image, frame_points[-1], frame_points[0], color, 2, cv2.LINE_AA)
        for index, point in enumerate(frame_points):
            cv2.circle(image, point, 7, color, -1, cv2.LINE_AA)
            cv2.circle(image, point, 10, (22, 36, 44), 2, cv2.LINE_AA)
            cv2.putText(image, str(index + 1), (point[0] + 12, point[1] - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.62, (238, 248, 250), 2, cv2.LINE_AA)
        if label and frame_points:
            cv2.putText(image, label, (frame_points[0][0] + 12, frame_points[0][1] + 24),
    ...
    def draw_review_frame_sample_overlay(self, image: np.ndarray) -> None:
        ...
        label = f"Samples: {total_count} | Tray {item_count} | BG {background_count}"
        font = cv2.FONT_HERSHEY_SIMPLEX
        scale = max(0.55, min(0.95, image.shape[1] / 1800.0))
        thickness = 2
        text_size, baseline = cv2.getTextSize(label, font, scale, thickness)
    ...
    def update_view(self) -> None:
        ...
        if (
            ...
            and not self.yolo_preview_paused_for_depth_plane
            and self.active_yolo_model_path() is not None
        ):
            self.run_yolo_preview(display_frame)
            self.draw_yolo_preview_overlay(display_frame)
        else:
            self.yolo_preview_detections = []
        ...
        if self.tray_roi_selection_active and self.tray_roi_points:
            self.draw_point_polygon_overlay(display_frame, self.tray_roi_points, (42, 170, 255), "Tray ROI")
        elif self.has_manual_tray_roi() and self.active_bin is not None:
            self.draw_point_polygon_overlay(display_frame, self.active_bin.roi_points, (88, 224, 132), "Tray ROI")
        if self.tray_plane_points:
            plane_points = [
                (
                    int(np.clip(round(point[0]), 0, max(0, display_frame.shape[1] - 1))),
                    int(np.clip(round(point[1]), 0, max(0, display_frame.shape[0] - 1))),
                )
                for point in self.tray_plane_points
            ]
            for index, point in enumerate(plane_points):
                cv2.circle(display_frame, point, 7, (38, 226, 236), -1, cv2.LINE_AA)
                cv2.circle(display_frame, point, 10, (12, 88, 96), 2, cv2.LINE_AA)
                cv2.putText(display_frame, str(index + 1), (point[0] + 12, point[1] - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.62, (238, 248, 250), 2, cv2.LINE_AA)
            if len(plane_points) >= 2:
                for start, end in zip(plane_points, plane_points[1:]):
                    cv2.line(display_frame, start, end, (38, 226, 236), 2, cv2.LINE_AA)
                if len(plane_points) == 4:
                    cv2.line(display_frame, plane_points[-1], plane_points[0], (38, 226, 236), 2, cv2.LINE_AA)
    ...
    def draw_panel(self, canvas: np.ndarray) -> None:
        ...
        cv2.putText(panel, "Tray Name", (PANEL_PAD, y), cv2.FONT_HERSHEY_SIMPLEX,
        ...
        roi_label = "Teach ROI"
        if self.tray_roi_selection_active:
            roi_label = f"ROI {len(self.tray_roi_points)}/4"
        elif self.has_manual_tray_roi():
            roi_label = "ROI Set"
        self.draw_button(
            panel,
            "teach_tray_roi",
            ...
            roi_label,
            not training_active and not self.recording_active,
            self.tray_roi_selection_active,
            role="save" if self.has_manual_tray_roi() else "default",
        )
        y += BUTTON_HEIGHT + 10
        ...
        record_enabled = self.recording_active or (
            self.has_item_name() and self.has_manual_tray_roi() and not training_active
        )
        ...
        cv2.putText(panel, f"Tray {self.sample_count}", (PANEL_PAD, y + 18),
        ...
        if training_active:
            ...
            self.draw_progress_button(
                ...
                self.sample_count > 0 and self.has_item_name() and self.has_manual_tray_roi() and not self.recording_active)
        else:
            self.draw_button(panel, "train_yolo", train_rect, "Train YOLO11",
                             self.sample_count > 0 and self.has_item_name() and self.has_manual_tray_roi() and not self.recording_active)
        y += BUTTON_HEIGHT + 10
        plane_label = "Teach Plane"
        if self.tray_plane_selection_active:
            plane_label = f"Teach Plane {len(self.tray_plane_points)}/4"
        elif self.has_tray_plane_reference():
            plane_label = f"Plane {self.tray_width_mm:.0f}x{self.tray_height_mm:.0f}mm"
        plane_enabled = (
            not training_active
            and not self.recording_active
            and self.has_item_name()
            and self.has_manual_tray_roi()
            and self.active_yolo_model_path() is not None
        )
        self.draw_button(
            panel,
            ...
            (PANEL_PAD, y, LEFT_PANEL_WIDTH - 2 * PANEL_PAD, BUTTON_HEIGHT),
            plane_label,
            plane_enabled or self.tray_plane_selection_active,
            self.tray_plane_selection_active,
            role="save" if self.has_tray_plane_reference() else "default",
        )
        y += BUTTON_HEIGHT + 8
    ...
    def commit_item_name_edit(self) -> None:
        ...
        if not safe_name(new_name):
            self.status = "Tray name cannot be empty" if not new_name else "Tray name needs letters or numbers"
            self.item_name_edit_buffer = self.item_name
            self.item_name_edit_active = False
            return
        if new_name != self.item_name:
            if self.training_thread is not None and self.training_thread.is_alive():
                self.status = "Cannot change tray name while training"
                self.item_name_edit_buffer = self.item_name
                self.item_name_edit_active = False
                return
            if self.recording_active:
                self.status = "Cannot change tray name while capturing ROI"
                self.item_name_edit_buffer = self.item_name
                self.item_name_edit_active = False
                return
    ...
    def cancel_item_name_edit(self) -> None:
        self.item_name_edit_buffer = self.item_name
        self.item_name_edit_active = False
        self.status = "Tray name edit canceled"
    ...
    def draw_video_bar(self, canvas: np.ndarray) -> None:
        ...
        status = "Tray ROI set" if self.has_manual_tray_roi() else "Tray ROI not set"
        if self.recording_active:
            status = f"Capturing ROI | {self.recording_frame_count} frames"
        elif self.review_mode:
            status = f"Reviewing ROI image | {self.review_status_label()}"
        if self.tray_roi_selection_active:
            status += f" | ROI {len(self.tray_roi_points)}/4"
        elif self.active_bin:
            status += f" | {self.active_bin.bin_name}"
        if self.preview_source_size != (0, 0):
            status += f" | {self.preview_source_size[0]}x{self.preview_source_size[1]}"
        if self.active_yolo_model_path() is not None:
            status += f" | {fit_text(self.yolo_preview_status, 34)}"
        if self.tray_plane_selection_active:
            status += f" | plane {len(self.tray_plane_points)}/4"
        elif self.has_tray_plane_reference():
            status += f" | plane {self.tray_width_mm:.0f}x{self.tray_height_mm:.0f}mm"
        if self.predicting:
            status += " | SAM2 running"
        cv2.putText(canvas, status, (LEFT_PANEL_WIDTH + 16, 74), cv2.FONT_HERSHEY_SIMPLEX,
                    0.56, (216, 224, 232), 1, cv2.LINE_AA)
    ...
    def bin_dropdown_label(self) -> str:
        return "Tray ROI set" if self.has_manual_tray_roi() else "Teach tray ROI"
    ...
    def mouse_callback(self, event, x: int, y: int, flags, param) -> None:
        ...
        if event == cv2.EVENT_LBUTTONDOWN and self.handle_button_click(x, y):
            return
        if self.handle_tray_roi_click(event, x, y):
            return
        if self.handle_tray_plane_click(event, x, y):
            return
    ...
    def handle_button_click(self, x: int, y: int) -> bool:
        ...
        for name, button in list(self.buttons.items()):
            ...
            if bx <= x <= bx + bw and by <= y <= by + bh:
                if not button.enabled:
                    return True
                if name == "teach_tray_roi":
                    self.start_tray_roi_selection()
                elif name == "bin_dropdown":
                    if self.recording_active:
                ...
                elif name == "teach_tray_plane":
                    self.start_tray_plane_selection()
        ...
        if ix <= x <= ix + iw and iy <= y <= iy + ih:
            ...
            self.item_name_edit_buffer = self.item_name
            self.status = "Type tray name, Enter to apply, Esc to cancel"
            return True
